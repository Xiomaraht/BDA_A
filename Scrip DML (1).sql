...
/*--27--*/
Select f.nombre, count(p.codigo) from fabricante f left join
producto p on f.codigo_fabricante = p.codigo_fabricante and p.precio>=220
group by f.nombre;

/*--28--*/
Select distinct f.nombre from fabricante f join producto p on f.codigo_fabricante = p.codigo_fabricante
group by f.codigo_fabricante, f.nombre
having sum(p.precio) >1000;

/*--29--*/
Select p.nombre, p.precio, f.nombre from producto p join fabricante f on p.codigo_fabricante = f.codigo_fabricante
where(p.codigo_fabricante, p.precio) in (select codigo_fabricante, max(precio) from producto group by codigo_fabricante)
order by codigo;
...
/*--9--*/
select nombre, codigo_fabricante from fabricante where codigo_fabricante =any (select codigo_fabricante from producto);

/*--2--*/
select nombre, codigo_fabricante from fabricante where codigo_fabricante <>  all (select codigo_fabricante from producto);

/*--1.1.7.3--*/

/*--11--*/
select nombre from fabricante where codigo_fabricante in ( select codigo_fabricante from producto);

/*--12--*/
select nombre from fabricante where codigo_fabricante not in ( select codigo_fabricante from producto);

/*--1.1.7.4--*/

/*--13--*/
select nombre from fabricante f where exists (select 1 from producto p where p.codigo_fabricante = f.codigo_fabricante);

/*--14--*/
select nombre from fabricante f where not exists (select 1 from producto p where p.codigo_fabricante = f.codigo_fabricante);

/*--1.1.7.5--*/

/*--15--*/
select f.nombre, p.nombre, p.precio from fabricante f join producto p on p.codigo_fabricante =f.codigo_fabricante
where p.precio = (select max(p2.precio) from producto p2 where p2.codigo_fabricante = f.codigo_fabricante);

/*--16--*/
select * from producto p1
where p1.precio >=(select avg(p2.precio) from producto p2 where p2.codigo = p1.codigo);
